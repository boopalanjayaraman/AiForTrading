...
def idiosyncratic_var_matrix(returns, factor_returns, factor_betas, ann_factor):
    ...
    #TODO: Implement function
    comm_returns_arr = np.dot(factor_returns, factor_betas.T)
    
    comm_returns = pd.DataFrame(comm_returns_arr, returns.index, returns.columns)
    spec_returns = returns - comm_returns
    
    idiosyncratic_var_matrix = pd.DataFrame(np.diag(np.var(spec_returns)) * ann_factor, returns.columns, returns.columns )
...
def predict_portfolio_risk(factor_betas, factor_cov_matrix, idiosyncratic_var_matrix, weights):
    ...
    #TODO: Implement function
    b_f_bt = np.dot(np.dot(factor_betas, factor_cov_matrix), factor_betas.T)
    b_f_bt_s = b_f_bt + idiosyncratic_var_matrix
    
    full_product = np.dot(np.dot(weights.T, b_f_bt_s), weights)
    predicted_portfolio_risk = np.sqrt(full_product)
...
# 
# **Answer:** 
# Momentum factor in the FRA chart already looks almost like a horizontal line. Smoothing is done so the factor does not react to incorrect signals (outliers / noise). So the answer should be no major change. 
...
class OptimalHoldings(AbstractOptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        #TODO: Implement function
        objective = cvx.Minimize(- np.array(alpha_vector.T) * weights)
        return objective
    ...
    def _get_constraints(self, weights, factor_betas, risk):
        ...
        #TODO: Implement function
        constraints = [risk <= self.risk_cap**2, np.array(factor_betas.T) * weights <= self.factor_max, 
                       np.array(factor_betas.T) * weights>= self.factor_min, sum(weights) ==0, sum(cvx.abs(weights)) <= 1,
                       weights >= self.weights_min, weights <= self.weights_max]
        return constraints
...
class OptimalHoldingsRegualization(OptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        #TODO: Implement function
        objective = cvx.Minimize(- np.array(alpha_vector.T) * weights + self.lambda_reg * cvx.norm(weights, 2))
        return objective
...
class OptimalHoldingsStrictFactor(OptimalHoldings):
    def _get_obj(self, weights, alpha_vector):
        ...
        #TODO: Implement function
        alpha_mean = np.mean(np.array(alpha_vector))
        target_weights = (np.array(alpha_vector) - alpha_mean)/np.sum(np.abs(np.array(alpha_vector)))
        
        #print(target_weights)
        
        objective = cvx.Minimize(cvx.norm(weights - target_weights.flatten()))
