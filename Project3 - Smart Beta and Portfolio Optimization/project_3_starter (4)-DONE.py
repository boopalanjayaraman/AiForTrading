...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    #TODO: Implement function
    all_rebalance_weights_original = pd.DataFrame(all_rebalance_weights)
    all_rebalance_weights_prev = all_rebalance_weights_original.shift(1)
    rebalance_weights_diff = np.abs(all_rebalance_weights_prev - all_rebalance_weights_original)
    sum_total_turnover = np.sum(rebalance_weights_diff).sum()
        
    number_of_rebal_events_per_year = n_trading_days_in_year / shift_size
    portfolio_turnover = sum_total_turnover * number_of_rebal_events_per_year / rebalance_count
